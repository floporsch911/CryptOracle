...
export default function FinancialChart() {
  ...
  const getAnteriorData = async (endTime) => {
    ...
    const response = await axios.get(
      `https://api.binance.com/api/v3/klines?symbol=${selectedSymbol}&interval=${interval}&startTime=${startTime.getTime()}&endTime=${endTimeDate.getTime()}`
    );
  ...
  Pour récupérer les données de bougies de 1 heure pour une période spécifique.
     */
    const response = await axios.get(
      `https://api.binance.com/api/v3/klines?symbol=${selectedSymbol}&interval=${interval}&limit=500`
    );
    return response.data.map((item) => ({
      time: item[0] / 1000,
      open: parseFloat(item[1]),
