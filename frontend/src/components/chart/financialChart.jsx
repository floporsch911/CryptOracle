...
export default function FinancialChart({ selectedSymbol, setselectedSymbol }) {
  ...
  const connectionWebSocketData = () => {
    const socket = new WebSocket(`wss://stream.binance.com:9443/ws/${selectedSymbol.symbol.toLowerCase()}@kline_${interval}`);
  ...
  const getAnteriorData = async (endTime) => {
    ...
    const response = await axios.get(
      `/api/binance/data?symbol=${selectedSymbol.symbol}&interval=${interval}&startTime=${startTime.getTime()}&endTime=${endTimeDate.getTime()}`
    );
  ...
  const getInitialData = async () => {
    const response = await axios.get(
      `/api/binance/data?symbol=${selectedSymbol.symbol}&interval=${interval}`
    );
    return response.data.data
  };
